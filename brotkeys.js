...
class HotkeyManager {
	...
	*/
	constructor(wordMap, interruptMap, ignoreTheseKeys){
        ...
        this.iframebodies_that_we_autogenerated_within = new Set();
	...
	autogenerateWithinId(containerID, generationTarget, /*optional*/ css_class_name, /*optional*/ arbitrary_swap_class_name) {
        ...
        keepTrackOfIframesWeAutogeneratedWithin(document, containerID);
	...
	static showKeys(pls_show_keys, of_class){
        ...
        for (let iframebody of this.iframebodies_that_we_autogenerated_within) {
            if (iframebody != document) {
                // because we already handled the document above
                let elems = Array.from(iframebody.getElementsByClassName(of_class));
                for (elem of elems) { elem.style.display=new_display;}
    ...
    static keepTrackOfIframesWeAutogeneratedWithin(doc, id){
        let elem = doc.getElementById(id);
        if (elem.tagName == "IFRAME") {
            this.iframebodies_that_we_autogenerated_within.add(elem);.add(elem.contentWindow.document);
        }
    }
